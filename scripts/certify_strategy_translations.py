"""Certifica le traduzioni delle strategie, lingua per lingua.
...
L'italiano non passa di qui: lo certifica `certify_seeded_strategy_drafts.py`
insieme alla riga.
...
`DATABASE_URL` puntato alla porta pubblicata da Postgres (5435).
    ...
    DATABASE_URL=postgresql://USER:PASS@127.0.0.1:5435/DB \
        python3 -m scripts.certify_strategy_translations --dry-run
    DATABASE_URL=... python3 -m scripts.certify_strategy_translations --apply
"""
...
def main() -> int:
    ...
    try:
        rows = (
            ...
            .order_by(models.CertifiedStrategy.id)
            .all()
        )
        promoted = Counter()
        skipped = Counter()
        for row in rows:
            for lang in TOOL_TARGET_LANGS:
                version = get_version(db, "certified_strategy", row.slug, lang)
        ...
        print(f"\n{total} versioni {'promosse' if args.apply else 'da promuovere'}"
              f" su {len(rows)} strategie certificate")
        return 0
    finally:
        db.close()
