...
try {
    for (const lang of locales) {
        ...
        await page.route('**/*', async route => {
            ...
            if (path === '/auth/me') data = { authenticated: true, is_admin: false, is_researcher: false, username: teacher ? 'teacher.demo' : 'student.demo', name: 'Alex · Demo', groups: [teacher ? 'docenti' : 'studenti'] };
        ...
        async function go(path) { await page.goto(`${origin}${path}`, { waitUntil: 'networkidle' }); await page.locator('main h1').waitFor(); }
