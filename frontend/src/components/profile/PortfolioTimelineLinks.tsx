...
type Link = { session_id: string; event_id: string; title: string; timeline_title: string };
export function PortfolioTimelineLinks({ itemId, description, locale }: { itemId: number; description?: string | null; locale: string }) {
    ...
    return <>
        ...
        {data?.links?.map(link => <a key={`${link.session_id}:${link.event_id}`} className="block min-h-[44px] py-3 text-sm text-indigo-700 underline" href={`/profilo/timeline?session=${encodeURIComponent(link.session_id)}&event=${encodeURIComponent(link.event_id)}`}>{l('backTimeline')}: {link.title} · {link.timeline_title}</a>)}
    </>;
}
