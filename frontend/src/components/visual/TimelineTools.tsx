    ...
    return <fieldset disabled={pending} className="min-w-0 space-y-4">
        ...
        <ol className={personal ? "space-y-4" : "ml-2 space-y-4 border-l-2 border-indigo-200 pl-4"}>
            {timeline.events.filter(event => !personal || activeEvent === event.id).map((event, index) => { const Icon = symbols[event.symbol]; return <li key={event.id} tabIndex={-1} id={`timeline-${event.id}`} className="relative min-w-0 rounded-xl border border-slate-200 bg-white p-3">
                ...
                {personal && <div className="my-3 flex flex-wrap gap-4">{(['notebook', 'orientation'] as const).map(key => <label key={key} className="flex min-h-11 items-center gap-2 text-sm"><input type="checkbox" checked={(event.personal_links || []).includes(key)} onChange={e => updateEvent(event.id, { personal_links: e.target.checked ? [...(event.personal_links || []), key] : (event.personal_links || []).filter(link => link !== key) })} />{l(key)}</label>)}</div>}
                {(event.personal_links || []).filter(key => key !== 'booklet').map(key => <Link key={key} className="mr-4 inline-block min-h-11 py-2 text-indigo-700 underline" href={`/profilo/${{ notebook: 'taccuino', orientation: 'orientamento' }[key]}`}>{l(key)}</Link>)}
