...
export function GoalForm({ form, setForm, groups }: FormProps) {
    ...
    return <div className="space-y-4">
        ...
        {(['motivation', 'criteria'] as const).map(key => <Field key={key} label={l(key)}><textarea className={input} rows={3} maxLength={key === 'criteria' ? 1500 : 2000} value={form[key]} onChange={e => setForm({ ...form, [key]: e.target.value })} /></Field>)}
