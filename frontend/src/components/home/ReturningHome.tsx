...
export function ReturningHome({
    ...
    return (
        <div className="flex flex-col gap-5 py-2">
                            ...
                            <div className="mt-3 grid gap-3 md:grid-cols-2">
                                    ...
                                    return (
                                        <article key={q.id} className="glass-panel flex flex-col gap-3 p-5 text-left transition-colors hover:border-indigo-300">
                                            ...
                                            <div className="flex flex-wrap items-center gap-2 pt-1">
                                                ...
                                                <button type="button" onClick={() => onStartInstrument(q)} className={cn('inline-flex min-h-[44px] items-center rounded-md px-3.5 py-2 text-sm font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400 focus-visible:ring-offset-2', canCompleteQuestionnaire ? 'border border-slate-200 bg-white text-slate-700 hover:bg-slate-50' : 'bg-indigo-600 text-white hover:bg-indigo-700')}>
                                                ...
                                                <Link href={`/strumenti/${q.id}`} className="inline-flex min-h-[44px] items-center gap-1.5 rounded-md px-3 py-2 text-sm font-medium text-slate-600 transition-colors hover:bg-slate-50 hover:text-indigo-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400 focus-visible:ring-offset-2">
                                                    <BookOpen className="h-4 w-4" />{t('selector.learn')}
                                                </Link>
                                            </div>
                                        </article>
                                    );
                                ...
                                {group.standaloneIds.includes('pqbl') && (
                                    <article className="glass-panel flex flex-col gap-3 p-5 text-left transition-colors hover:border-indigo-300">
                                        <div className="flex flex-wrap items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-ochre-500" /><h3 className="font-bold text-slate-900">{t('pqbl.card.title')}</h3><span className="rounded-full bg-ochre-50 px-2 py-0.5 text-2xs font-bold text-ochre-700">{t('pqbl.card.badge')}</span></div>
                                        <p className="grow text-sm leading-relaxed text-slate-500">{t('pqbl.card.desc')}</p>
                                        <div className="pt-1"><Link href="/pqbl" className="inline-flex min-h-[44px] items-center rounded-md bg-indigo-600 px-3.5 py-2 text-sm font-semibold text-white transition-colors hover:bg-indigo-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400 focus-visible:ring-offset-2">{t('pqbl.card.cta')}</Link></div>
                                    </article>
                                )}
                            </div>
                        </section>
            ...
            <header>
                <h2 className="font-display text-3xl font-bold text-slate-900">{t('base.title')}</h2>
            ...
            {/* Kept as a guarded fallback while the catalog above is the visible home. */}
            {false && <section>
                <div className="flex flex-wrap items-center justify-between gap-2">
