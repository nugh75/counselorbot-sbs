...
export function AssignmentsPanel({ teacher = false, showHeading = true }: { teacher?: boolean; showHeading?: boolean }) {
    ...
    return <section aria-label={l(teacher ? 'sent' : 'received')} className="space-y-4">
        ...
        {!teacher && <p className="text-sm text-slate-600">{learningText(lang, 'visibility')}</p>}
        ...
        {!failed && rows.map(row => <article key={row.id} id={`assignment-${row.id}`} className="scroll-mt-24 space-y-3 break-words rounded-xl border border-slate-200 bg-white p-4 sm:p-5">
            ...
            <p className="text-sm font-medium text-indigo-700">{learningText(lang, row.intent || 'proposal')}{row.due_date && <> · <time dateTime={row.due_date}>{row.due_date}</time></>}</p>
            {row.response_prompt && <p className="whitespace-pre-wrap text-sm"><strong>{learningText(lang, 'responsePrompt')}: </strong>{row.response_prompt}</p>}
            ...
            {!teacher && <AssignmentWork assignmentId={row.id} />}
