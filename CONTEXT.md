...
- **Taccuino del docente (teacher notebook)**: the teacher's self-declared notes about their ROLE — subjects, experience, classroom methodologies, classes and institutes, professional development interests (`TeacherProfileRevision`, `/user/teacher-notebook` API, editable in `/docente`). Append-only like the student notebook; no autosave. It enters the OBIETTIVO_DOCENZA chat by default, in place of the student taccuino/portfolio/goals; the teacher can switch the notebook in context from the chat options (student / teacher / none) for any instrument, and the server re-checks the teacher role each turn. A teacher's student-side data, if any, never enters the teacher-notebook conversation.
...
- **Teacher-area entry**: `/docente` mirrors the personal area: the OBIETTIVO_DOCENZA path on top, three illustrated groups (classroom, catalogs, research) linking to `/docente/<slug>` subpages (`classi`, `assegnazioni`, `catalogo-obiettivi`, `strategie`, `materiali`, `orientamento`, `somministrazioni`; `teacher-area.ts`, six-language `i18n-teacher-area.ts`), and the teacher Notebook inline. Subpages share `useTeacherAccessState` (same `canUseTeacherAssistant` guard) and `TeacherAreaHeader` (back link to `/docente`); the Orientamento card stays teacher-only (`isTeacher`), as before. Panels keep their own inner headings.
