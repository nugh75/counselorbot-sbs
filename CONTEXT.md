...
- **Thread guard**: `thread_guard.py` judges each finished counselor turn on a preset of its own (`thread_guard_preset_id`, seeded by `thread_guard_seed.py` to a local qwen3.8 with thinking off; the admin panel “Guardiano del filo” can point it at any preset, external providers included) and stores the verdict in `Log(action="thread_guard")`. Four fields — `on_thread`, `question_fit`, `advice_grounded`, `answered.last_question_developed` — and only the failing ones are injected: on an ordinary turn as a `[THREAD]` block of at most 400 characters, at step entry folded into `session_ledger` under its 1900-character cap so nothing is said twice. It judges the counselor and never the student: a student who changes subject is not derailing, only a counselor who neither followed it nor tied it back is. Notes are facts, never orders — an imperative note or one carrying “should” is dropped, a rambling one keeps its first sentence, a long one is trimmed at a word boundary. At most two check notes per turn in fixed severity order (`advice_grounded`, `on_thread`, `question_fit`); a note identical to the previous turn's is suppressed once, not forever. The deterministic half stays where it already was: the age of an open question and a repeated step come from `session_ledger`, catalogue membership from `recommendation_blocks`, and the model is asked only for relevance, grounding and whether the thread held. The worker is a daemon thread started after the turn is logged in both the buffered and the streaming branch, with its own session and its own PII redaction (`redact_always`, since the judge may be external). A verdict about an older exchange is stale and ignored, and timeout, unreadable JSON, a missing preset or `thread_guard_enabled` false each render an empty block — never a delayed or failed turn. Off by default: the verdicts are meant to be read in the logs before the injection is switched on. The Compass does not pass through `chat_preparation` and is not covered. See `docs/plans/thread-guard.md`.
- **Advice and questions**: a third sidebar tab, “Consigli e domande”, uses `RecommendationHistory` type `advice` with payload kind `advice` or `question`. A private `recommendations.notes` declaration is accepted only for text present in the visible completed reply; advice respects the step gate. Exact normalized duplicates share a stable hash. The student's question state is `proposed` (open) or `closed`, with explicit reopening; neither advancement nor the model closes it. The latest 12 notes (220 characters each) and any explicitly named older note are injected as data to avoid paraphrased repeats, and closed questions are excluded from the ledger's open-question reminder. Semantic non-repetition depends on the model and must be observed live. Books and certified strategies keep their existing catalogues and states.
