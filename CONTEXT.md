...
- **Skills engine**: `backend/skills/` selects and renders the skills bound to the current step. Deterministic filter on declarative `conditions`, then an LLM router only when the optional candidates exceed `skills_router_threshold` (deterministic fallback on error or timeout). It is active for all seven supported instruments; `certified-advice` is the sole strategy-advice skill, while `approved-strategies` is retained but inactive and unbound. The historic path in `chat_logic._retrieved_context` remains the rollback when the global flag is turned off.
