...
- **Personal timeline**: the Timeline tab in `VisualTools` stores an ordered list of past/future milestones in the existing versioned workspace. Periods are student-written and may be approximate; arrow controls set the order. Milestones link same-session action IDs (including `book`, `article`, and `film` goals) and owned Portfolio work IDs. Titles of linked works are resolved on read/save; unavailable references expose no content. Links never duplicate or delete the source work. The student can select milestones for the chat composer or preview a dated Portfolio copy, using `/session/{session_id}/visual-tools/timeline/preview` and `/timeline/portfolio`. The copy respects the existing 4000-character Portfolio limit without truncation, checks preview/revision freshness, and records an idempotency token in `Log(action="timeline_snapshot")`; it does not synchronize with later edits. `/user/portfolio/{item_id}/timeline-links` resolves links back to available owned milestones at `/profilo/timeline?session=…&event=…`. Saved timelines appear in standalone and final PDFs. Tests: `backend/tests/test_timeline.py` and the timeline cases in `frontend/tests/visual-tools.test.mjs`; the optional real-API browser fixture is `python -m backend.tests.timeline_browser_server` with `DATABASE_URL` configured, followed from `frontend/` by `TIMELINE_LIVE=1 node --test --experimental-strip-types --test-name-pattern="timeline real API" tests/visual-tools.test.mjs`. The fixture uses a rolled-back schema in `counselorbot_test` and listens only on loopback.
