...
- **Diagram icon meanings**: `diagram_icon_catalog.json` defines 100 allowed IDs and English meanings, shared by manual generation and the `concept-diagram` skill. Nodes may omit icons independently, especially for abstract self-beliefs or misleading achievement symbols; their shapes and labels remain. Original assets/specs remain valid. New Lucide vectors and both PNG themes are generated with `node scripts/generate_diagram_icons.mjs`; `--check` verifies assets and frontend parity. The English skill cap is 3900; the SHA-256-guarded `skills_diagram_semantic_icons_v1` migration preserves custom text and activation state. See `docs/diagram-icons.md` and its searchable preview.
