...
from .routes import audio_input as audio_input_routes
...
def _run_seed_and_migrations():
    ...
    try:
        ...
        # Migration: add assistant_audience column.
        try:
            with database.engine.connect() as conn:
                conn.execute(sa_text(
                    "ALTER TABLE counselors ADD COLUMN assistant_audience VARCHAR"
                ))
                conn.commit()
        except Exception as e:
        ...
        for key, default, descr in [
            ...
            ("monthly_budget_usd", "0", "Budget mensile in USD; raggiunto il limite registrato si usano solo modelli locali o OpenRouter gratuiti (0 = nessun limite)."),
            ("ai_fallback_targets", "[]", "Ripieghi in ordine: lista JSON di provider e model, massimo tre. Il modello scelto resta il primo tentativo; i servizi a pagamento si fermano quando la spesa registrata raggiunge il budget mensile."),
            ("model_context_profiles", "{}", "Profili JSON per provider/modello: context_tokens, input_tokens e compact. Usare limiti verificati sul server e sul compito."),
            ("ai_timeout_seconds", "120", "Timeout di lettura per tentativo AI, da 10 a 600 secondi."),
            ("omniroute_url", "http://omniroute:20128/v1", "Endpoint OpenAI compatibile OmniRoute; OMNIROUTE_API_URL nell'ambiente prevale."),
        ...
        try:
            ...
            i18n_changed = apply_reading_and_translations_policy(db)
            ...
            obiettivo_engine_changed = apply_obiettivo_engine_policy(db)
            ...
            if (configs_changed or skills_changed or policy_changed or specialized_changed
                    ...
                    or budget_changed or event_paths_changed or obiettivo_engine_changed):
        ...
        # successiva dell'admin resta.
        from .counselor_scope import extend_interview_counselors, extend_obiettivo_counselors
        extend_interview_counselors(db)
        extend_obiettivo_counselors(db)
...
_COUNSELOR_PERSONA_EN_BY_SLUG = {
    ...
    "iride": (
        "You are {{counselor_name}} (Iride), a synthetic assistant for students. You "
...
def _seed_assistant_counselors(db):
    ...
    Rimuove eventuali vecchi counselor con slug obsoleti."""
        ...
        if slug in existing_slugs:
            ...
            if c:
                ...
                if persona and c.persona != persona:
                    c.persona = persona
                    changed = True
                audience = cfg.get("assistant_audience")
...
app.include_router(audio_input_routes.router)
