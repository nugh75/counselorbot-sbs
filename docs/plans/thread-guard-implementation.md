...
- Note text is English, one factual sentence, ≤140 characters, no imperatives.
...
- [x] Tests: a healthy verdict renders `""`; two failing checks render a
      ...
      JSON, on a missing field and on a non-boolean `ok`; a note written as an
      order is dropped; a rambling note keeps its first sentence and a long one
      is trimmed at a word boundary; `answered.last_question_developed=false`
      adds its own line.
- [x] Implement, run `pytest backend/tests/test_thread_guard.py -v`, commit.
...
- [x] Tests (`artifact_session`): a stored verdict comes back for its own turn
...
- [x] Implement over `models.Log(action="thread_guard")`, run tests, commit.
...
- [x] Tests: the built input carries the step prompt, the student's last
...
- [x] Tests: `enabled` false makes `evaluate` return without calling the model
...
- [x] Tests: with a stored verdict, an ordinary turn's system prompt contains
...
- [x] `session_ledger.build` accepts `guard_notes`, `_compose` renders them
...
- [x] Run `pytest backend/tests/test_thread_guard.py backend/tests/test_session_ledger.py backend/tests/test_chat_preparation.py -v`, commit.
...
- [x] Test: the seed picks an existing active ollama `qwen3.8` preset with
...
- [x] Panel: an on/off switch and a preset dropdown fed by `/api/admin/presets`
...
- [x] Run the backend tests and `npx tsc --noEmit`, commit.

### Task 7: close out

- [x] `python -m backend.tests.test_smoke` inside the backend container.
- [x] `docker compose up -d --build`, check status and logs.
- [x] Update `CONTEXT.md` with the thread guard entry.
- [x] Push the branch.
